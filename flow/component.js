...
declare interface Component {
  ...
  // public properties
  $el: any; // so that we can attach __vue__ to it 当前组件的真实dom

  $data: Object; //data

  $props: Object; //props

  $options: ComponentOptions;//vue  的自定义项

  $parent: Component | void;//父组件

  $root: Component; //根节点

  $children: Array<Component>;//子组件

  $refs: { [key: string]: Component | Element | Array<Component | Element> | void }; //refs对象

  $slots: { [key: string]: Array<VNode> }; //插槽

  $scopedSlots: { [key: string]: () => VNodeChildren };//作用域插槽

  $vnode: VNode; // the placeholder node for the component in parent's render tree //虚拟节点

  $attrs: { [key: string] : string }; //属性

  $listeners: { [key: string]: Function | Array<Function> }; //监听事件

  $isServer: boolean; //是否是服务端渲染

  // public methods
  $mount: (el?: Element | string, hydrating?: boolean) => Component; //挂载组件

  $forceUpdate: () => void;//强制刷新

  $destroy: () => void; //销毁当前组件

  $set: <T>(target: Object | Array<T>, key: string | number, val: T) => T; //设置属性

  $delete: <T>(target: Object | Array<T>, key: string | number) => void; //删除属性 触发更新
